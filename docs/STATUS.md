...
- Login and session-restore screens rebuilt on the design system's own materials - the drifting mesh, glass, and the Fraunces display the central `service-auth` sign-in page uses - so leaving for SSO and coming back reads as one continuous product. The screen in use is concept 7, "The Index, Lit": a printed index drifting floor to ceiling with one band cut edge to edge through it. The authenticated shell carries the same *ambient shell, solid content* language it was built on (technical design section 11.2). Earlier concepts remain parked as a design study at `/designs/login/1..4`.
...
- Manual (`workflow_dispatch`) production deployment workflow, kept manual by choice now that provisioning is complete.
